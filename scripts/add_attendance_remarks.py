...
replace_first(
    ...
    '''    return {
      date:d,
      status,
      holiday: "holiday" in (rec || {}) ? (rec as any).holiday : holidays.find(item => item.date === iso) ?? null,
...
# Reflect remarks in the formal monthly PDF generated by monthly_attendance_pdf.py.
replace_once(
    '''      let specialLabel = "";
...
# Add remarks to calendar hover text when a normal attendance date has one.
