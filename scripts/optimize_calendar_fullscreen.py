...
def replace_global_once(old: str, new: str, label: str):
    global text
    if old not in text:
...
def replace_calendar_once(old: str, new: str, label: str):
    global calendar
    if old not in calendar:
        raise RuntimeError(f'Could not locate Calendar {label}.')
    calendar = calendar.replace(old, new, 1)


# Every structural replacement below is deliberately scoped to CalendarView.
for old, new, label in [
    ('<div className="it-calendar-stage max-w-4xl mx-auto space-y-4">', '<div className="it-calendar-stage w-full min-h-full">', 'stage'),
    ('<section className="relative overflow-hidden rounded-[30px] sm:rounded-[38px] px-4 py-5 sm:px-8 sm:py-7"', '<section className="it-calendar-shell relative overflow-hidden min-h-[calc(100svh-4.75rem-env(safe-area-inset-bottom))] md:min-h-screen rounded-none md:rounded-[30px] px-4 pt-[max(1rem,env(safe-area-inset-top))] pb-6 sm:px-6 md:px-8 md:py-6"', 'shell'),
    ('<div className="relative z-10">', '<div className="it-calendar-layout relative z-10 min-h-full">', 'layout'),
    ('<div className="flex items-center justify-between text-white mb-6">', '<div className="it-calendar-topbar flex items-center justify-between text-white mb-4 sm:mb-5">', 'top bar'),
    ('<div className="text-center px-2">', '<div className="it-calendar-hero text-center px-1 sm:px-2">', 'hero'),
    ('<div className="grid grid-cols-2 gap-3 sm:gap-4 mt-6 max-w-lg mx-auto">', '<div className="it-calendar-day-controls grid grid-cols-2 gap-2.5 sm:gap-4 mt-4 sm:mt-5 max-w-lg mx-auto">', 'day controls'),
    ('<div className="text-center mt-10 mb-1">', '<div className="it-calendar-focus text-center mt-6 sm:mt-8 mb-0">', 'focus summary'),
    ('<div className="relative mt-1">', '<div className="it-calendar-dial relative mt-0">', 'dial'),
    ('<div className="grid grid-cols-3 gap-3 max-w-xl mx-auto -mt-4 sm:-mt-7">', '<div className="it-calendar-actions grid grid-cols-3 gap-2 sm:gap-3 max-w-xl mx-auto -mt-5 sm:-mt-8">', 'actions'),
]:
    replace_calendar_once(old, new, label)

sheet_marker = '<section className="rounded-[26px] p-5 sm:p-6"'
if calendar.count(sheet_marker) < 2:
    raise RuntimeError('Could not locate both Calendar sheets.')
...
style_block = r'''  return (
    <>
      <style>{`
        ...
        .it-calendar-shell { isolation: isolate; }
        ...
        @media (max-width: 430px) {
          .it-calendar-main { height:100svh; }
          .it-calendar-shell {
          ...
          .it-calendar-topbar { margin-bottom:0.55rem; }
          ...
          .it-calendar-focus { margin-top:1rem !important; }
          .it-calendar-focus p:nth-child(2) { font-size:2.65rem !important; }
          .it-calendar-dial { margin-top:-0.45rem !important; }
          .it-calendar-dial svg {
        ...
        @media (min-width: 768px) {
          ...
          .it-calendar-shell { min-height:calc(100dvh - 1.6rem) !important; }
          .it-calendar-layout {
            display:grid;
          ...
          .it-calendar-topbar { grid-column:1/-1; grid-row:1; }
          .it-calendar-hero { grid-column:1; grid-row:2; align-self:end; }
          ...
          .it-calendar-stage:has(.it-calendar-sheet) .it-calendar-shell { min-width:0; }
          .it-calendar-sheet {
            align-self:stretch;
        ...
        @media (min-width: 1440px) {
          .it-calendar-main { padding:1rem !important; }
          .it-calendar-shell { min-height:calc(100dvh - 2rem) !important; }
          .it-calendar-layout {
            grid-template-columns:minmax(340px,0.78fr) minmax(520px,1.22fr);
            column-gap:clamp(2rem,4vw,5rem);
          }
          .it-calendar-hero h2 { font-size:clamp(4.5rem,6vw,6.5rem) !important; }
        }
      `}</style>
      <div className="it-calendar-stage w-full min-h-full">'''

if return_marker not in calendar:
    raise RuntimeError('Could not find Calendar return root for CSS insertion.')
calendar = calendar.replace(return_marker, style_block, 1)

close_marker = '    </div>\n  );\n}\n\n'
if close_marker not in calendar:
    raise RuntimeError('Could not find CalendarView closing block.')
calendar = calendar.rsplit(close_marker, 1)[0] + '      </div>\n    </>\n  );\n}\n\n'
